...
local local_poly := realroot(diff(poly, x), 1/10000);
    DEBUG(__FILE__, __LINE__, lprint(">> local_poly", evalf(local_poly)));
local curr_point;
local i, j := 1;
    for i from 1 to nops(S) do
        interval := bound_info(x, S[i], 0);
        DEBUG(__FILE__, __LINE__, lprint(">> Current interval", interval));
...
$endif
        ...
        DEBUG(__FILE__, __LINE__, lprint(">> curr_point", curr_point));
        while j <= nops(local_poly) and evalf(curr_point <= interval[2]) do
            curr_point := (local_poly[j,1]+local_poly[j,2])/2;
            DEBUG(__FILE__, __LINE__, lprint(">> Show evalf", evalf(subs(x=curr_point, poly)) ));
...
local num_roots := nops(roots_poly);
    DEBUG(__FILE__, __LINE__, lprint(">> poly", poly));
    DEBUG(__FILE__, __LINE__, lprint(">> roots_poly", roots_poly));
local curr_point;
...
local interval;
    for i from 1 to nops(S) do
        interval := bound_info(x, S[i], 0);
        DEBUG(__FILE__, __LINE__, lprint(">> Current interval", evalf(interval)));
        ...
        DEBUG(__FILE__, __LINE__, lprint(">> curr_min", curr_min));
        ...
        while j <= num_roots and evalf(roots_poly[j] < interval[1]) do
            DEBUG(__FILE__, __LINE__, lprint(">> j @1", j));
            j := j + 1;
        end do;

        while j <= num_roots and evalf(roots_poly[j] < interval[2]) do
            DEBUG(__FILE__, __LINE__, lprint(">> j @2", j));
            curr_point := evalf(subs(x=convert(roots_poly[j], rational), poly));
            if evalf(curr_point <= curr_min) then
                curr_min := curr_point;
                DEBUG(__FILE__, __LINE__, lprint(">> curr_arg_min", evalf(roots_poly[j])));
                DEBUG(__FILE__, __LINE__, lprint(">> curr_min", evalf(curr_min)));
            end if;
            j := j + 1;
        end do;
    end do;
